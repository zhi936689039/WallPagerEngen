package com.oyzb.wallpaper.view;
...
public class CornerImageView extends AppCompatImageView {
    ...
    public CornerImageView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        strokeColor = typedArray.getColor(R.styleable.OvalView_stroke_color_1, Color.TRANSPARENT);
        strokeSize = typedArray.getDimension(R.styleable.OvalView_stroke_size_1, 0);

        isOval = typedArray.getBoolean(R.styleable.OvalView_is_oval_1, false);
        if (isOval)
            return;

        isRoundedRectangle = typedArray.getBoolean(R.styleable.OvalView_is_round_rectangle_1, false);
        if (isRoundedRectangle)
            return;

        radius = typedArray.getDimension(R.styleable.OvalView_all_radius_1, 0);
        if (radius != 0) {
            isAllRound = true;
        } else {
            ul_radius = typedArray.getDimension(R.styleable.OvalView_ul_radius_1, 0);
            ur_radius = typedArray.getDimension(R.styleable.OvalView_ur_radius_1, 0);
            ll_radius = typedArray.getDimension(R.styleable.OvalView_ll_radius_1, 0);
            lr_radius = typedArray.getDimension(R.styleable.OvalView_lr_radius_1, 0);
        }
    }
